...
# |                          CERTEUS                                    |
# +=====================================================================+
# | FILE: scripts/remote-bot/setup.sh                                  |
# | ROLE: Shell script for automation                                  |
# | PLIK: scripts/remote-bot/setup.sh                                  |
...
ROOT=""
for up in "" ".." "../.." "../../.."; do
  CAND="$DIR/$up/workspaces/certeus/tools/remote-bot/setup.sh"
  if [ -f "$CAND" ]; then ROOT="$DIR/$up"; break; fi
done
if [ -z "$ROOT" ]; then
  echo "Nie znaleziono workspaces/certeus/tools/remote-bot/setup.sh względem $DIR" >&2
  exit 2
fi
exec "$ROOT/workspaces/certeus/tools/remote-bot/setup.sh" "$@"

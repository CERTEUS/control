...
def main() -> int:
    ...
    if not pem_path:
        # Default to enterprise key location
        default_pem = root / "internal" / "keys" / "github-app-private-key.pem"
        if default_pem.exists():
            pem_path = str(default_pem)

    if not app_id or not inst_id or not pem_path:
        msg = "Missing GH_APP_ID/GH_APP_INSTALLATION_ID/GH_APP_PRIVATE_KEY_PATH"
    ...
    r.raise_for_status()
    print(r.json()["token"])  # noqa: T201  # Script output for token
    return 0
